...
function MonthlyRevenueChart({ rows }: { rows: MonthlyRevenue[] }) {
  ...
  return (
    <>
      ...
      <div style={{ width: "100%", height: 260 }}>
        <ResponsiveContainer>
          <ComposedChart data={data} margin={{ top: 16, right: 36, left: 0, bottom: 8 }}>
            ...
            />
            <Bar yAxisId="left" dataKey="revenue_b" name="Revenue" fill="#4d8bff" radius={[3, 3, 0, 0]} />
            <Line
              yAxisId="right"
              type="monotone"
              dataKey="yoy"
      ...
      <div className="stock-chart-legend">
        <LegendItem color="#4d8bff" label="Monthly revenue (NT$ B)" />
...
function QuarterlyFinancialsChart({ rows }: { rows: QuarterlyFinancials[] }) {
  ...
  return (
    <>
      ...
      <div className="stock-chart-legend">
        <LegendItem color="#4d8bff" label="Revenue (NT$ B)" />
